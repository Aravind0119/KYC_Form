...
# Read Wiiz webhook from Render environment variable
...
if not WIIZ_WEBHOOK_URL:
    raise ValueError("WIIZ_WEBHOOK_URL environment variable is not set")


# 🔹 Function to send data to Wiiz in background (non-blocking)
...
# 🔹 When Wiiz opens the URL → show form
...
def submit():
    ...
    # Send data to Wiiz asynchronously (prevents timeout)
    threading.Thread(target=send_to_wiiz, args=(payload,)).start()

    # Immediate response (VERY IMPORTANT to avoid timeout)
    return """
    <html>
